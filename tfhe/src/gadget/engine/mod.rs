...
impl BooleanEngine {
    ...
    fn encryption_from_plaintext(&mut self, cks : &ClientKey, plaintext : Plaintext<u64>) -> LweCiphertext<Vec<u64>>{
    ...
    //     };
    //     let plain =  Plaintext(buffer as u64);
    //     let ct = self.encryption_from_plaintext(cks, plain);
    //     Ciphertext::BooleanEncrypted(ct, encoding.clone())
    // }


    pub fn encrypt_arithmetic(&mut self, message : u64, encoding : &Encoding, cks : &ClientKey) -> Ciphertext{
    ...
    // fn decrypt_boolean(lwe_sk : &LweSecretKey<&[u64]>, ciphertext : &LweCiphertext<Vec<u64>>, encoding : &BooleanEncoding) -> u64{
    //     // decryption
    ...
    //     let closest_integer = (decrypted_u64 as f64 * slice).round() as u64 % encoding.get_modulus();
    ...
    fn decrypt_arithmetic(lwe_sk : &LweSecretKey<&[u64]>, ciphertext : &LweCiphertext<Vec<u64>>, encoding : &Encoding) -> u64{
        // decryption
        let decrypted = decrypt_lwe_ciphertext(&lwe_sk, ciphertext);
    ...
    pub fn measure_noise(&mut self, ct: &Ciphertext, cks: &ClientKey) -> i64 {
        match ct {
            ...
            Ciphertext::EncodingEncrypted(ciphertext, encoding) => {
                ...
                if noise.abs() > encoding.get_modulus() as f64 / 2.0 {noise = encoding.get_modulus() as f64 - noise.abs()}
...
impl BooleanEngine{
        ...
        input.iter().enumerate().for_each(|(i, x)| {
            match x {
                ...
                Ciphertext::Trivial(x_bool) => {
                    ...
                    lwe_ciphertext_plaintext_add_assign(&mut buffer_lwe_before_pbs, Plaintext(plaintext as u64));
                }
            }
        });
    ...
    pub fn apply_lut(&mut self, input : &Ciphertext, output_encoding : &Encoding, f : &dyn Fn(u64) -> u64, server_key : &ServerKey) -> Ciphertext{
        match input{
    ...
    pub fn mvb(&mut self, input : &Ciphertext, output_encodings : &Vec<Encoding>, lut_fis : &Vec<Vec<u64>>, server_key : &ServerKey) -> Vec<Ciphertext>{
        match input {
            Ciphertext::EncodingEncrypted(c, input_encoding) => {
                ...
                match server_key.pbs_order {
                    ...
                    PBSOrder::KeyswitchBootstrap =>{
                        ...
                        let cis: Vec<LweCiphertext<Vec<u64>>> = bootstrapper.mvb_bootstrap(c_after_ks,  input_encoding, output_encodings, lut_fis, server_key);
                        cis.iter()
                            .zip(output_encodings)
    ...
    pub fn compute_common_factor(&mut self, ciphertext:&Ciphertext, enc_out : &Encoding, server_key : &ServerKey) -> GlweCiphertextOwned<u64>{
       let bootstrapper = &mut self.bootstrapper;
        ...
        match ciphertext{
            Ciphertext::EncodingEncrypted(lwe_ciphertext, _) =>{
                ...
                bootstrapper.bootstrap_common_factor(
                    &c_after_ks,
                    enc_out,
                    &server_key
                    )
    ...
    pub fn simple_tree_bootstrapping(&mut self, common_factor : &GlweCiphertextOwned<u64>, inputs : &Vec<Ciphertext>, encoding_out : &Encoding, t : u64, lut_fi : Vec<u64>, server_key : &ServerKey, client_key_debug : &ClientKey, log : bool) -> Ciphertext{
            ...
            Ciphertext::EncodingEncrypted(lwe_c_0, encoding_in_0) => {
                ...
                match server_key.pbs_order{
                    ...
                    PBSOrder::KeyswitchBootstrap =>{
                        ...
                        let first_ciphertexts = bootstrapper.mvb_bootstrap_with_common_factor_given(&common_factor, lwe_c_0_after_ks, &encoding_in_0, &vec![encoding_out.clone();(t/o_0).try_into().unwrap()], &first_functions, &server_key, &client_key_debug);
    ...
    pub fn encoding_switching_mul_constant(&mut self, input : &Ciphertext, coefficient : u64, server_key : &ServerKey) -> Ciphertext{
    ...
    pub fn simple_plaintext_sum(&mut self, input : &Ciphertext, constant : u64, modulus : u64, server_key : &ServerKey) -> Ciphertext{
    ...
    pub fn simple_mul_constant(&mut self, input : &Ciphertext, constant : u64, modulus : u64, server_key : &ServerKey) -> Ciphertext{
    ...
    pub fn encoding_switching_sum_constant(&mut self, input : &Ciphertext, constant : u64, modulus : u64, server_key : &ServerKey) -> Ciphertext{
