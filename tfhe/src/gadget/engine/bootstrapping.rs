...
use crate::core_crypto::prelude::{CiphertextModulus, ContiguousEntityContainer, ContiguousEntityContainerMut, Fft, MonomialDegree, PBSOrder};
...
impl Memory {
    ...
    fn allocate_ciphertexts_for_bootstrapping(&mut self, server_key: &ServerKey)->(GlweCiphertext<&mut[u32]>, LweCiphertext<&mut[u32]>, LweCiphertext<&mut[u32]>){
    ...
    pub fn as_buffers_common_factor(
        ...
        let constant = if enc_out.get_modulus() % 2 == 0{1u64 << 31} else {1u64 << 32}; 

        accumulator.get_mut_body().as_mut().fill((constant / (enc_out.get_modulus() as u64)) as u32);   //filling the common factor with ones (no taking into account the tau factor here) (il faudra mettre un scaling sinon on va se noyer dans le bruit)
...
pub struct ServerKey {
    ...
    pub(crate) lwe_packing_keyswitch_key : LwePackingKeyswitchKeyOwned<u32>,
...
impl Bootstrapper {
    ...
    pub(crate) fn bootstrap(
        ...
        accumulator : &GlweCiphertext<Vec<u32>>,
        server_key: &ServerKey) -> LweCiphertextOwned<u32> {
        // let BuffersRef {
        //     lookup_table: accumulator,
        ...
        let mut buffer_lwe_after_pbs = LweCiphertext::new(0u32, server_key
            .bootstrapping_key
            .output_lwe_dimension()
        ...
        let mut output_glwe = GlweCiphertext::new(0u32, accumulator.glwe_size(), accumulator.polynomial_size(), accumulator.ciphertext_modulus());
    ...
    fn bootstrap_common_factor(
        ...
        let mut output = GlweCiphertext::new(0u32, accumulator.glwe_size(), accumulator.polynomial_size(), accumulator.ciphertext_modulus());
    ...
    fn create_vi_for_mvb(
        ...
        let mut accumulator =  Polynomial::new(0u32, PolynomialSize(N_poly));
        ...
        for i in 0usize..p-1{
            let diff = (accumulator_data[i+1] as i32 - accumulator_data[i] as i32).rem_euclid(new_p as i32) as u32;
            accumulator[N_poly / (2 * p) + i * N_poly / p] = diff;
        }
        let diff = (new_p as i32 - accumulator_data[0] as i32 - accumulator_data[p-1] as i32).rem_euclid(new_p as i32) as u32;
    ...
    // where         OutputCont: Container<Element = u32>,
    // {
    ...
    //         let mut output_lwe = LweCiphertext::new(0u32, lwe_size, CiphertextModulus::new_native());
    ...
    pub(crate) fn mvb_bootstrap(
        ...
        let mut result = vec![];
        for vi in vis{
            let mut accu_i = GlweCiphertext::new(0u32, v0.glwe_size(), v0.polynomial_size(), v0.ciphertext_modulus());
            ...
            //Sample Extraction
            let mut output_lwe_i = LweCiphertext::new(0u32, accu_i.glwe_size().to_glwe_dimension().to_equivalent_lwe_dimension(accu_i.polynomial_size()).to_lwe_size(), accu_i.ciphertext_modulus());
    ...
    pub(crate) fn pack_into_new_accumulator(
        &self, 
        lwe_ciphertexts : Vec<LweCiphertextOwned<u32>>,
        server_key : &ServerKey,
        p : u32,
    ) -> GlweCiphertext<Vec<u32>>{
        let mut output_glwe_ciphertext = GlweCiphertext::new(0u32, server_key.bootstrapping_key.glwe_size(), server_key.bootstrapping_key.polynomial_size(), CiphertextModulus::new_native());
        ...
        let accumulator_elements : Vec<_> = (0..p).map(|k|{
            if k % 2 == 0{ 
                match lwe_ciphertexts.get(k as usize/2){
                    Some(c) => c.clone(),
                    None => LweCiphertext::new(0u32, lwe_ciphertexts[0].lwe_size(), CiphertextModulus::new_native())
            }}
            else{
